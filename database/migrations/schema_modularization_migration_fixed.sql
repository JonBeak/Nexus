-- Schema Modularization Migration Script (Fixed for Triggers)
-- Separates the monolithic sign_manufacturing database into 7 feature-specific schemas
-- Handles triggers and constraints properly
-- 
-- Created: 2025-09-10
-- Author: Claude Code
...
-- =============================================================================
-- STEP 2: BACKUP AND RECREATE TRIGGERS
-- =============================================================================

-- Store trigger definitions
CREATE TEMPORARY TABLE temp_triggers AS 
SELECT 
    TRIGGER_NAME,
    EVENT_MANIPULATION,
    EVENT_OBJECT_TABLE,
...
WHERE TRIGGER_SCHEMA = 'sign_manufacturing';

-- Drop existing triggers
DROP TRIGGER IF EXISTS sign_manufacturing.set_customer_address_sequence;
DROP TRIGGER IF EXISTS sign_manufacturing.customer_create_trigger;
...
ALTER TABLE sign_manufacturing.suppliers 
...
UPDATE sign_manufacturing.suppliers 
SET supplier_type = 'vinyl' 
WHERE supplier_id IN (
    SELECT DISTINCT supplier_id 
    FROM sign_manufacturing.vinyl_inventory 
    WHERE supplier_id IS NOT NULL
);

-- =============================================================================
-- STEP 4: MOVE TABLES TO APPROPRIATE SCHEMAS (Without Triggers)
-- =============================================================================
...
-- CUSTOMERS SCHEMA TABLES (Move these first without triggers)
...
-- Move customers and customer_addresses last (these had triggers)
...
DELIMITER $$
CREATE TRIGGER customers.set_customer_address_sequence
BEFORE INSERT ON customers.customer_addresses
FOR EACH ROW
BEGIN
...
CREATE TRIGGER customers.customer_create_trigger
AFTER INSERT ON customers.customers
FOR EACH ROW
BEGIN
    INSERT INTO customers.customer_history (customer_id, field_name, old_value, new_value, change_type, changed_by, change_reason)
    VALUES (NEW.customer_id, 'customer_record', NULL, 'Customer created', 'create', NEW.created_by, 'New customer added to system');
    
    INSERT INTO customers.customer_communication_preferences (customer_id)
...
VALUES (NULL, 'SCHEMA_MIGRATION', 'DATABASE', 'ALL_SCHEMAS', 
        'Completed database schema modularization migration with trigger handling', NOW());
