-- Schema Modularization Migration Script (Safe Version)
-- Separates the monolithic sign_manufacturing database into 7 feature-specific schemas
-- Handles existing columns, triggers, and constraints safely
-- 
-- Created: 2025-09-10
-- Author: Claude Code
...
-- Check if supplier_type column already exists, if not add it
SET @col_exists = (
    SELECT COUNT(*) 
...
-- Add constraint to ensure vinyl inventory only uses vinyl suppliers (safely)
SET @constraint_exists = (
    SELECT COUNT(*) 
    FROM information_schema.TABLE_CONSTRAINTS 
    WHERE CONSTRAINT_SCHEMA = 'vinyl' 
    AND TABLE_NAME = 'vinyl_inventory' 
    AND CONSTRAINT_NAME = 'chk_vinyl_supplier_type'
);
...
SET @sql = IF(@constraint_exists = 0, 
    ...
    'SELECT ''Constraint chk_vinyl_supplier_type already exists'' as message'
);
...
-- Create view for vinyl suppliers only (drop first if exists)
DROP VIEW IF EXISTS vinyl.vinyl_suppliers;
CREATE VIEW vinyl.vinyl_suppliers AS 
SELECT * FROM supply_chain.suppliers 
WHERE supplier_type IN ('vinyl', 'both');

-- Create view for general suppliers only (drop first if exists)
...
-- Create consolidated customer view with address info (drop first if exists)
...
VALUES (NULL, 'SCHEMA_MIGRATION', 'DATABASE', 'ALL_SCHEMAS', 
        'Completed database schema modularization migration (safe version)', NOW());
