...
export class VinylProductsRepository {
  ...
  static async createVinylProduct(data: VinylProductData, supplierIds?: number[]): Promise<number> {
    ...
    try {
      ...
      if (supplierIds && supplierIds.length > 0) {
        await this.updateProductSuppliers(productId, supplierIds);
      }
  ...
  static async updateVinylProduct(
    ...
    try {
      ...
      if (fields.length > 0) {
        await this.syncProductChangesToInventory(productId, data);
      }
  ...
  static async hardDeleteVinylProduct(productId: number): Promise<boolean> {
    ...
    try {
      ...
      // Delete the product
      const result = await query('DELETE FROM vinyl_products WHERE product_id = ?', [productId]) as any;
  ...
  static async findOrCreateProductFromInventory(inventoryData: {
    ...
    const productData: VinylProductData = {
      ...
      colour_number: inventoryData.colour_number || null,
      colour_name: inventoryData.colour_name || null,
      default_width: inventoryData.width || null,
      is_active: true,
      created_by: inventoryData.created_by || null,
      updated_by: inventoryData.created_by || null
    };
