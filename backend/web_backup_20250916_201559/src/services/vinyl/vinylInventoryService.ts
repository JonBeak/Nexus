...
export class VinylInventoryService {
  ...
  static async createVinylItem(
    ...
    try {
      ...
      const vinylData: VinylInventoryData = {
        brand: data.brand,
        series: data.series,
        colour_number: data.colour_number || null,
        colour_name: data.colour_name || null,
        width: data.width,
      ...
      const vinylId = await VinylInventoryRepository.createVinylItem(vinylData);

      // Handle job associations if provided
      if (data.job_ids && data.job_ids.length > 0) {
        await VinylInventoryRepository.updateJobLinks(vinylId, data.job_ids);
      }

      // Auto-create product catalog entry if it doesn't exist
      let productId: number | undefined;
      try {
        productId = await VinylProductsRepository.findOrCreateProductFromInventory({
          brand: data.brand,
          series: data.series,
          colour_number: data.colour_number,
          colour_name: data.colour_name,
          width: data.width,
          created_by: user.user_id
        });
      } catch (error) {
        // Product creation is not critical, continue without failing
        console.warn('Failed to auto-create product:', error);
      }
  ...
  private static validateVinylItemData(data: CreateVinylItemRequest): {
    ...
    if (data.purchase_date && data.purchase_date.trim() !== '' && isNaN(new Date(data.purchase_date).getTime())) {
      errors.push('Invalid purchase date');
    }

    if (data.storage_date && data.storage_date.trim() !== '' && isNaN(new Date(data.storage_date).getTime())) {
      errors.push('Invalid storage date');
    }

    if (data.expiration_date && data.expiration_date.trim() !== '' && isNaN(new Date(data.expiration_date).getTime())) {
