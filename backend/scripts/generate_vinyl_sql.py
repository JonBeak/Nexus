#!/usr/bin/env python3
"""
Generate SQL script to import vinyl inventory data
"""
...
def main():
    ...
    print("-- SQL Script to Import Vinyl Inventory")
    print("-- Generated automatically from inventory data")
    print("USE sign_manufacturing;")
    print("")
    
    # Track unique products for bulk insert
    unique_products = set()
    vinyl_items = []
    ...
    for line in lines:
        ...
        product_key = (parsed['brand'], parsed['series'], parsed['colour'], parsed['width'])
        unique_products.add(product_key)
        vinyl_items.append(parsed)
    
    # Generate product inserts
    print("-- Insert unique products")
    for brand, series, colour, width in sorted(unique_products):
        brand_esc = escape_sql_string(brand)
        series_esc = escape_sql_string(series)
        colour_esc = escape_sql_string(colour)
        
        print(f"INSERT IGNORE INTO vinyl_products (brand, series, colour, default_width, is_active, created_at, updated_at)")
        print(f"VALUES ('{brand_esc}', '{series_esc}', '{colour_esc}', {width}, 1, NOW(), NOW());")
    
    print("")
    print("-- Insert vinyl inventory items")
    
    # Generate vinyl inventory inserts
    for item in vinyl_items:
        brand_esc = escape_sql_string(item['brand'])
        series_esc = escape_sql_string(item['series'])
        colour_esc = escape_sql_string(item['colour'])
        
        print(f"INSERT INTO vinyl_inventory (brand, series, colour, width, length_yards, disposition, storage_date, location, created_at, updated_at)")
        print(f"VALUES ('{brand_esc}', '{series_esc}', '{colour_esc}', {item['width']}, {item['length_yards']}, 'in_stock', CURDATE(), 'Storage', NOW(), NOW());")
    
    print("")
    print("-- Summary")
    print(f"-- Total unique products: {len(unique_products)}")
    print(f"-- Total vinyl items: {len(vinyl_items)}")
