...
export class JobEstimationService {
  ...
  async getAddonTypes(): Promise<RowDataPacket[]> {
    try {
      return await this.repository.getAddonTypes();
    } catch (error) {
      console.error('Service error fetching addon types:', error);
      throw new Error('Failed to fetch addon types');
    }
  }
