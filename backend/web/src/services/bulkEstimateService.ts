...
export const createBulkEstimate = async (
  ...
  try {
    ...
    const addonMappings: Array<{ temp_id: string; actual_id: number }> = [];
      ...
      for (let j = 0; j < group.items.length; j++) {
        ...
        const [itemResult] = await connection.execute<ResultSetHeader>(
          `INSERT INTO job_estimate_items 
           (group_id, product_type_id, item_name, item_order, input_data, 
        ...
        // 4. Create all addons for this item
        for (let k = 0; k < item.addons.length; k++) {
          const addon = item.addons[k];
          const addonOrder = k + 1;

          const [addonResult] = await connection.execute<ResultSetHeader>(
            `INSERT INTO job_item_addons 
             (item_id, addon_type_id, addon_order, input_data, quantity, unit_price, extended_price) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [
              actualItemId,
              addon.addon_type_id,
              addonOrder,
              JSON.stringify(addon.input_data),
              1, // Default quantity
              25.00, // Temporary addon price
              25.00
            ]
          );

          const actualAddonId = addonResult.insertId;
          addonMappings.push({ temp_id: addon.temp_id, actual_id: actualAddonId });
        }
      }
    }
