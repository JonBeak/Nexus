...
export class OrderRepository {
  ...
  async updateOrder(orderId: number, data: UpdateOrderData): Promise<void> {
    ...
    if (data.high_standards !== undefined) {
      updates.push('high_standards = ?');
      params.push(data.high_standards === null ? null : data.high_standards ? 1 : 0);
    }
