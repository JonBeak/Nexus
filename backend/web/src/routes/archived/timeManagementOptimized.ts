  ...
  try {
    ...
    switch (dataType) {
      ...
      case 'summary':
        ...
        const summaryParams: (string | number)[] = [startDate as string, endDate as string];
        if (group && group !== 'all' && group !== '') {
          summaryParams.push((group === 'Group A' || group === 'Group B') ? String(group) : Number(group as string));
        }
