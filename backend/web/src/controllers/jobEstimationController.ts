...
// Get addon types
export const getAddonTypes = async (req: Request, res: Response) => {
  try {
    const data = await jobEstimationService.getAddonTypes();
    res.json({ success: true, data });
  } catch (error) {
    console.error('Controller error fetching addon types:', error);
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to fetch addon types'
    });
  }
};
