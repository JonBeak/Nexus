...
export const ordersApi = {
  ...
  /**
   * Update specs quantity for a part (Phase 1.5.g)
   * @param orderNumber - Order number
   * @param partId - Part ID to update
   * @param specsQty - Manufacturing quantity
   */
  ...
  /**
   * Get available images in order folder (Phase 1.5g)
   * @param orderNumber - Order number
   */
  ...
  /**
   * Set job image for order (Phase 1.5g)
   * @param orderNumber - Order number
   * @param filename - Image filename to set
   * @param cropCoords - Optional crop coordinates (pixels to trim from each edge)
   */
  async setJobImage(
    orderNumber: number,
    filename: string,
