 ...
 *
 * Combined panel showing "To Be Copied" and "Current Order Parts" side by side.
 * Features:
...
export const CombinedStagingTargetPanel: React.FC<CombinedStagingTargetPanelProps> = ({
  ...
  const totalSlots = useMemo(() => {
    const maxStagedSlot = stagedRows.length > 0
      ...
      : -1;
    return Math.max(targetParts.length, maxStagedSlot + 1);
  }, [stagedRows, targetParts]);
  ...
  const handleDrop = useCallback((e: React.DragEvent, slotIndex: number) => {
    ...
    if (stagedRowsData) {
      try {
        ...
        console.error('Failed to parse staged rows data:', err);
      }
      setDragOverSlot(null);
      setDraggedRowIds(new Set());
      setSelectedRowIds(new Set());
  ...
  const renderSlotRow = (slotIndex: number) => {
    ...
    const targetPart = targetParts[slotIndex];
    const isNewRow = slotIndex >= targetParts.length;
    ...
    return (
      ...
      >
        ...
        <div className={`flex shrink-0 relative ${isDraggedOver ? 'bg-blue-50' : ''} ${isBeingDragged ? 'opacity-50' : ''} ${isSelected ? 'before:absolute before:inset-0 before:ring-2 before:ring-inset before:ring-blue-600 before:z-50 before:pointer-events-none' : ''}`} style={{ width: 'calc(51% + 32px)' }}>
          {/* Slot Number */}
          ...
          {stagedRow ? (
            ...
            <div className={`flex-1 px-3 py-2 text-gray-400 italic text-center border-r border-gray-200 ${
              isDraggedOver ? 'bg-blue-100 text-blue-600' : 'bg-gray-50'
            }`}>
              ...
              <div className={`w-32 px-2 py-1.5 border-r border-gray-200 shrink-0 ${
                ...
                {qbItemWillChange ? (
                  <>
                    ...
                    {stagedRow!.data.qb_item_name !== targetPart.qb_item_name && (
                      <div className="text-gray-400 line-through">{targetPart.qb_item_name || '-'}</div>
                    )}
                  </>
                ) : (
                  targetPart.qb_item_name || <span className="text-gray-400 italic">-</span>
                )}
              </div>
              ...
              <div className={`flex-1 min-w-0 px-2 py-1.5 border-r border-gray-200 ${
                ...
                <div style={{ wordBreak: 'break-word' }}>
                  {qbDescWillChange ? (
                    ...
                    renderWithNewlines(targetPart.qb_description)
                  )}
                </div>
                {qbDescWillChange && targetPart.qb_description && stagedRow!.data.qb_description !== targetPart.qb_description && (
              ...
              <div className={`w-10 px-2 py-1.5 text-center border-r border-gray-200 shrink-0 ${
                ...
                {qtyWillChange ? (
                  <>
                    ...
                    {stagedRow!.data.quantity !== targetPart.quantity && (
                      <div className="text-gray-400 line-through">{targetPart.quantity}</div>
                    )}
                  </>
                ) : (
                  targetPart.quantity
              ...
              <div className={`w-16 px-2 py-1.5 text-right shrink-0 ${
                ...
                {priceWillChange ? (
                  <>
                    ...
                    {stagedRow!.data.unit_price !== targetPart.unit_price && (
                      <div className="text-gray-400 line-through">{formatCurrency(targetPart.unit_price)}</div>
                    )}
                  </>
                ) : (
                  formatCurrency(targetPart.unit_price)
                )}
              </div>
            </>
          ) : (
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex border-b border-gray-200 bg-gray-100 text-xs sticky top-0 z-20 select-none">
          ...
          <div className="flex-1 py-1.5 px-2 text-gray-700 font-medium">
            Current Order Parts ({targetParts.length})
          </div>
        </div>
        ...
        <div className="flex border-b border-gray-300 text-xs font-medium sticky top-[26px] z-10 select-none">
          ...
          <div className="flex shrink-0" style={{ width: '51%' }}>
            ...
            <div className="w-40 px-2 py-1.5 text-gray-600 border-r border-gray-200 shrink-0 bg-gray-50">Source Item</div>
            <div
        ...
        >
          <div className={`w-8 px-1 py-2 text-center shrink-0 border-r border-gray-200 ${
            dragOverSlot === totalSlots ? 'bg-blue-100 text-blue-600' : 'text-gray-400'
          }`}>
            <Plus className="w-3 h-3 inline" />
          </div>
          <div className={`flex-1 px-3 py-2 text-gray-400 ${
            dragOverSlot === totalSlots ? 'bg-blue-100 text-blue-600' : ''
          }`}>
